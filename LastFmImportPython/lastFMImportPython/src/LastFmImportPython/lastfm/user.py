...
class User(LastfmBase):
    ...
    def get_recent_tracks_pages (self, limit = None, page=None, date_from = None,
            date_to = None):
        ...
        if date_from is not None:
            params.update({'from': date_from})
        if date_to is not None:
            params.update({'to': date_to})
        data = self._api._fetch_data(params, no_cache = True).find('recenttracks')
        
        return int(data.attrib['totalPages'])        
        
    def get_recent_tracks(self, limit = None, date_from = None,
            date_to = None, page = None):
        params = self._default_params({'method': 'user.getRecentTracks'})
        if limit is not None:
            params.update({'limit': limit})
        if page is not None:
        ...
        for t in data.findall('track'):
            if t.get('nowplaying'):
                continue
            tracks.append(lastFmTrackDetails(
                      name = t.findtext('name'),
                      ...
                      played_on_uts = t.find('date').get('uts'),
                      )
                )
        return tracks
...
class lastFmTrackDetails(object):
